...
position at time \(t_i\), \(\Delta_{t_i} = t_{i+1} - t_i\) is the time
interval between observations \(\mathbf{X}_{t_i}\) and
\(\mathbf{X}_{t_{i+1}}\), and \(D\) is the separation distance
introduced later in Section~\ref{sec-model-eval}.
...
\subsubsection{Model evaluation}\label{sec-model-eval}
...
separates four times slower than the drifter moves.
...
South-South-West and not captured by satellite-derived observations
...
panel of Figure~\ref{fig-lin-comb-est}) shows an overall very good
...
\textsubscript{Source:
\href{https://vadmbertr.github.io/otc25-final_report/sections/03_results-preview.html\#527b5cf9-3aa0-4dbc-ad8f-82ad0d7537bc}{Results}}
...
this terms from the drifter GPS data, but assessing their estimated
